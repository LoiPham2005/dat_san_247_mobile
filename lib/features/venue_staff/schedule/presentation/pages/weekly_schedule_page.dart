...
class _WeeklySchedulePageState extends State<WeeklySchedulePage> {
  ...
  DateTime _getWeekStart(DateTime d) => DateTime(d.year, d.month, d.day - (d.weekday - 1));
  ...
  void _today() {
    HapticFeedback.selectionClick();
    setState(() => _weekStart = _getWeekStart(DateTime.now()));
  }
  ...
  Map<String, Map<String, List<_SlotBooking>>> _buildMockData() {
    ...
    void add(String courtId, DateTime date, String start, String end, String name, String code,
        BookingStatusVS status) {
      final dk = DateFormat('yyyy-MM-dd').format(date);
      data.putIfAbsent(courtId, () => {});
      data[courtId]!.putIfAbsent(dk, () => []);
      data[courtId]![dk]!.add(
          _SlotBooking(start: start, end: end, customerName: name, bookingCode: code, status: status));
    }
    ...
    add('c2', today.add(const Duration(days: 1)), '07:00', '09:00', 'Vũ Phúc', 'DS006',
        BookingStatusVS.CONFIRMED);
    add('c3', today.subtract(const Duration(days: 1)), '06:00', '07:30', 'Bùi Hoàng', 'DS007',
        BookingStatusVS.COMPLETED);
    add('c1', today.add(const Duration(days: 2)), '19:00', '21:00', 'Trần X', 'DS008',
        BookingStatusVS.CONFIRMED);
  ...
  Widget build(BuildContext context) {
    final displayedCourts =
        _selectedCourtId == null ? _courts : _courts.where((c) => c.id == _selectedCourtId).toList();
            ...
            child: Column(children: [
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  TextButton(
                      onPressed: _today,
                      child: const Text('Hôm nay', style: TextStyle(color: Colors.white70, fontSize: 12))),
                ]),
              ),
              // Week navigation
              Padding(
                ...
                child: Row(children: [
                  IconButton(
                      onPressed: _prevWeek, icon: const Icon(Icons.chevron_left_rounded, color: Colors.white)),
                  Expanded(
                    child: Row(
                        ...
                        children: _weekDays.map((d) {
                          ...
                          return GestureDetector(
                            onTap: () {},
                            child: Column(children: [
                              Text(DateFormat('E', 'vi').format(d),
                                  style: TextStyle(
                                      color: isT ? Colors.white : Colors.white60,
                                      fontSize: 10,
                  ...
                  IconButton(
                      onPressed: _nextWeek, icon: const Icon(Icons.chevron_right_rounded, color: Colors.white)),
                ]),
              ),
              // Court filter
              SingleChildScrollView(
                ...
                child: Row(children: [
                  ...
                  ..._courts.map((c) => _CourtPill(
                      label: c.name,
                      selected: _selectedCourtId == c.id,
                      brand: _brand,
    ...
    return SizedBox(
      ...
      child: Column(children: [
        ...
        Container(
          ...
          child: Row(children: [
            SizedBox(
                width: timeColW, child: const Center(child: Text('', style: TextStyle(fontSize: 10)))),
                        ...
                        child: Container(
                          ...
                          child: Center(
                              child: Text(DateFormat('E\ndd/MM', 'vi').format(d),
                                  style: TextStyle(
                                      fontSize: 9,
                                      color: isT ? _brand : AppColors.textHint,
                                      fontWeight: isT ? FontWeight.bold : FontWeight.normal),
        ...
        ...List.generate(_slots.length, (slotIdx) {
          ...
          return Row(children: [
            SizedBox(
                ...
                height: cellH,
                child: Center(
                    child: Text(slot, style: const TextStyle(fontSize: 9, color: AppColors.textHint)))),
            ...courts.map((court) {
              return Row(
                  children: _weekDays.map((d) {
                ...
                return GestureDetector(
                  onTap: matching.isNotEmpty
                      ? () => _showBookingTip(context, matching.first, court.name, d)
                      : null,
                  child: Container(
                    width: cellW,
                    height: cellH,
                    decoration: BoxDecoration(
                      color: matching.isNotEmpty
                          ? _slotColor(matching.first.status)
                          : (isT ? _brand.withOpacity(0.04) : Colors.white),
                      border: const Border(
                        ...
                        ? Padding(
                            ...
                            child: Text(
                              ...
                              style: TextStyle(
                                  fontSize: 8,
                                  fontWeight: FontWeight.bold,
                                  color: _slotTextColor(matching.first.status)),
                              maxLines: 1,
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Row(children: [
            ...
            Text(courtName,
                style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: _brand)),
            const Spacer(),
            ScheduleStatusBadge(status: b.status),
          ]),
          const SizedBox(height: 12),
          ScheduleBookingTipRow(icon: Icons.person_rounded, label: 'Khách', value: b.customerName),
          ScheduleBookingTipRow(
              icon: Icons.access_time_rounded, label: 'Giờ', value: '${b.start} – ${b.end}'),
          ScheduleBookingTipRow(
              icon: Icons.event_rounded,
              label: 'Ngày',
              value: DateFormat('EEEE, dd/MM', 'vi').format(date)),
          ScheduleBookingTipRow(
...
class _CourtInfo {
  final String id;
  final String name;
  const _CourtInfo(this.id, this.name);
}
