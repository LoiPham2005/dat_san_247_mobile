...
import 'package:dat_san_247_mobile/features/owner/dashboard/presentation/providers/dashboard_notifier.dart';
...
class OwnerDashboardPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: switch (state) {
        AsyncData(:final value) when value.venues.isEmpty =>
          const Center(child: Text('Bạn chưa có sân nào để quản lý')),
        AsyncData(:final value) => RefreshIndicator(
            onRefresh: () => notifier.refresh(),
            child: CustomScrollView(
              slivers: [
                _buildHeader(context, notifier, value),
                SliverPadding(
                  padding: const EdgeInsets.all(16),
                  sliver: SliverToBoxAdapter(
                      child: _buildBody(context, notifier, value)),
                ),
              ],
            ),
          ),
                      ...
                      if (data.venues.length > 1)
                        IconButton(
                          icon: const Icon(Icons.swap_horiz_rounded,
                              color: Colors.white),
                          onPressed: () =>
                              _showVenueSelector(context, notifier, data),
                        ),
                    ],
                  ),
                  const SizedBox(height: 16),
          ...
          children: [
            ...
            ...data.venues.map(
              (v) => ListTile(
                ...
                trailing: v.id == data.selectedVenue?.id
                    ? const Icon(Icons.check_circle, color: _brand)
                    : null,
                onTap: () {
                  notifier.selectVenue(v);
                  Navigator.pop(ctx);
                },
              ),
            ),
      ...
      children: [
        ...
        if (data.pendingBookings.isEmpty)
          const Center(
            child: Padding(
              ...
              child: Text('Không có đơn nào chờ xác nhận'),
            ),
          ),
        ...data.pendingBookings.map((b) => DashboardPendingBookingCard(
              booking: {
                ...
                'date': b.bookingDate != null
                    ? DateFormat('dd/MM/yyyy').format(b.bookingDate!)
                    : '...',
              ...
              onAccept: () => notifier.acceptBooking(b.id),
              onReject: () => notifier.rejectBooking(b.id),
            )),
        const SizedBox(height: 24),
        DashboardSectionHeader(
          ...
          color: AppColors.warning,
          action: 'Xem tất cả',
          onAction: () => _replyReview(
              context, '', data.selectedVenue?.id ?? '', data.selectedVenue?.name ?? ''),
        ),
        const SizedBox(height: 10),
        ...
        ...data.recentReviews.map(
          (r) => DashboardReviewCard(
            ...
            onReply: () => _replyReview(
                context, r.id, data.selectedVenue?.id ?? '', data.selectedVenue?.name ?? ''),
          ),
        ),
      ],
    );
  ...
  void _replyReview(BuildContext context, String id, String venueId, String venueName) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => OwnerReviewsPage(venueId: venueId, venueName: venueName),
      ),
    );
  }
}
