...
class _OwnerVerificationPageState extends State<OwnerVerificationPage> {
  ...
  // Mock: trạng thái hiện tại = REJECTED để demo full flow
  VenueVerificationModel? _currentVerification = VenueVerificationModel(
    id:'vv1', venueId:'v1', version:1,
    businessLicenseUrl:'', idCardFrontUrl:'', idCardBackUrl:'', ownerPhotoUrl:'',
    status:VerificationDocStatus.REJECTED,
    rejectionReason:'Giấy phép kinh doanh không rõ ràng. Vui lòng tải lại ảnh độ phân giải cao hơn. Ảnh CMND/CCCD mặt sau bị mờ.',
  ...
  // "Uploaded" files (mock local paths)
  ...
  bool get _allFilesSelected => _filePaths.values.every((v) => v != null);

  @override
  Widget build(BuildContext context) {
    final ver = _currentVerification;
    return Scaffold(
        ...
        SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
            ...
            if (ver?.isPending == true)
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(gradient: LinearGradient(colors: [_brand.withOpacity(0.05), _brand.withOpacity(0.12)]), borderRadius: BorderRadius.circular(14), border: Border.all(color: _brand.withOpacity(0.3))),
                  child: Row(children: [
                    Container(width: 44, height: 44, decoration: BoxDecoration(color: _brand.withOpacity(0.1), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.upload_file_rounded, color: Color(0xFF0891B2), size: 22)),
          ...
          if (_showForm) ...[
            ...
            _UploadFormCard(
              ...
              onFileChanged: (key, path) => setState(() => _filePaths[key] = path),
              onCancel: () => setState(() { _showForm = false; _filePaths.updateAll((_, __) => null); }),
              onSubmit: _allFilesSelected ? () => _submit() : null,
  ...
  Future<void> _submit() async {
    ...
    final newVersion = (_currentVerification?.version ?? 0) + 1;
    setState(() {
      _isSubmitting = false;
      _showForm = false;
      _currentVerification = VenueVerificationModel(
        id: 'vv$newVersion', venueId: widget.venueId, version: newVersion,
        businessLicenseUrl: _filePaths['business_license'] ?? '',
        idCardFrontUrl: _filePaths['id_card_front'] ?? '',
        idCardBackUrl: _filePaths['id_card_back'] ?? '',
        ownerPhotoUrl: _filePaths['owner_photo'] ?? '',
        status: VerificationDocStatus.PENDING,
        createdAt: DateTime.now(), updatedAt: DateTime.now(),
      );
      _filePaths.updateAll((_, __) => null);
    });
    HapticFeedback.mediumImpact();
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('✅ Đã gửi hồ sơ! Chờ xét duyệt 1-3 ngày.'), backgroundColor: AppColors.success));
  }
}
...
class _StatusCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final statusColor = switch (ver.status) {
      VerificationDocStatus.PENDING  => AppColors.warning,
      VerificationDocStatus.APPROVED => AppColors.success,
      VerificationDocStatus.REJECTED => AppColors.error,
    };
    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: statusColor.withOpacity(0.3)), boxShadow: [BoxShadow(color: statusColor.withOpacity(0.08), blurRadius: 12)]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(width: 44, height: 44, decoration: BoxDecoration(color: statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(12)), child: Center(child: Text(ver.status.emoji, style: const TextStyle(fontSize: 20)))),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(ver.status.label, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: statusColor)),
            Text('Hồ sơ phiên bản ${ver.version} • ${DateFormat('dd/MM/yyyy').format(ver.createdAt)}', style: const TextStyle(fontSize: 10, color: AppColors.textHint)),
          ])),
        ]),
        if (ver.isRejected && ver.rejectionReason != null) ...[
          const SizedBox(height: 12),
          Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: AppColors.error.withOpacity(0.05), borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.error.withOpacity(0.2))),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
class _UploadFormCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: AppColors.borderLight), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 8)]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: uploaded ? AppColors.success.withOpacity(0.04) : const Color(0xFFF8FAFC),
                borderRadius: BorderRadius.circular(10),
                border: Border.all(color: uploaded ? AppColors.success.withOpacity(0.4) : AppColors.borderLight),
              ),
              child: Row(children: [
                Container(width: 40, height: 40, decoration: BoxDecoration(color: (uploaded ? AppColors.success : AppColors.textHint).withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
  ...
  void _pickFile(BuildContext context, String key) {
    ...
    showModalBottomSheet(context: context, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
      builder: (ctx) => Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        ListTile(leading: const Icon(Icons.camera_alt_rounded, color: Color(0xFF0891B2)), title: const Text('Chụp ảnh'), onTap: () { Navigator.pop(ctx); onFileChanged(key, '/mock/photo_$key.jpg'); }),
        ListTile(leading: const Icon(Icons.photo_library_rounded, color: Color(0xFF0891B2)), title: const Text('Chọn từ thư viện'), onTap: () { Navigator.pop(ctx); onFileChanged(key, '/mock/gallery_$key.jpg'); }),
        ListTile(leading: const Icon(Icons.insert_drive_file_rounded, color: Color(0xFF0891B2)), title: const Text('Chọn file PDF'), onTap: () { Navigator.pop(ctx); onFileChanged(key, '/mock/doc_$key.pdf'); }),
        const SizedBox(height: 8),
      ])),
    );
  }
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ...[
        ...
        Container(width: 22, height: 22, decoration: BoxDecoration(color: (s.$4).withOpacity(0.1), shape: BoxShape.circle), child: Center(child: Text(s.$1, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: s.$4)))),
        const SizedBox(width: 10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
