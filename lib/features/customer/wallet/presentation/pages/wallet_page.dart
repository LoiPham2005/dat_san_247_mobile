...
class _WalletPageState extends State<WalletPage>
  ...
  List<TransactionModel> get _filtered {
    switch (_selectedFilter) {
      ...
      case 2:
        return _transactions.where((t) => t.type == TransactionType.PAYMENT).toList();
      case 3:
          ...
          child: SafeArea(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  const Text('Ví DatSan247', style: TextStyle(color: AppColors.white70, fontSize: 13)),
                    ...
                    children: [
                      Text(
                        ...
                        style: const TextStyle(
                            color: AppColors.white, fontSize: 32, fontWeight: FontWeight.w900),
                      ),
                      const Spacer(),
                      GestureDetector(
                        onTap: () {
                          HapticFeedback.lightImpact();
                          Clipboard.setData(ClipboardData(text: _wallet.balance.toString()));
                        },
                        child: Container(
                  ...
                  if (_wallet.lockedBalance > 0) ...[
                    const SizedBox(height: 4),
                    Row(children: [
                      const Icon(Icons.lock_outline_rounded, color: AppColors.white70, size: 13),
                      const SizedBox(width: 4),
                      Text('${fmt.format(_wallet.lockedBalance)} đang giữ',
                          style: const TextStyle(color: AppColors.white70, fontSize: 12)),
                      const SizedBox(width: 6),
                      Text('Khả dụng: ${fmt.format(_wallet.availableBalance)}',
                          style: const TextStyle(
                  ...
                  Row(
                    children: [
                      ...
                      _QuickAction(icon: Icons.history_rounded, label: 'Giao dịch', onTap: () {}),
                    ],
                  ),
                ],
              ),
  ...
  Widget _buildTransactionList(NumberFormat fmt) {
    if (_filtered.isEmpty) return SliverFillRemaining(child: _buildEmpty());
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        (ctx, i) {
          final t = _filtered[i];
          final showDateSeparator =
              i == 0 || !_isSameDay(_filtered[i - 1].createdAt, t.createdAt);
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              if (showDateSeparator) _DateSeparator(date: t.createdAt),
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16),
                child: TransactionTile(
                  transaction: t,
                  fmt: fmt,
                  onTap: () => _showTransactionDetail(context, t, fmt),
                ),
              ),
            ],
          );
          ...
          child: Column(
            ...
            children: [
                ...
                children: amounts.map((a) {
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Text(NumberFormat.compactCurrency(locale: 'vi_VN', symbol: 'đ').format(a),
                          style: TextStyle(
                              fontSize: 13,
              ...
              const Text('Qua cổng thanh toán',
                  style:
                      TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: AppColors.textSecondary)),
              const SizedBox(height: 10),
              Row(
                children: [
                  _PaymentOption(
                      label: 'MoMo', color: const Color(0xFFAD1457), icon: Icons.phone_android_rounded),
                  ...
                  const SizedBox(width: 10),
                  _PaymentOption(
                      label: 'ZaloPay', color: const Color(0xFF0068FF), icon: Icons.payment_rounded),
                ],
              ),
              const SizedBox(height: 16),
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Text(
                    selected != null ? 'Nạp ${fmt.format(selected)}' : 'Xác nhận nạp tiền',
  ...
  void _showTransactionDetail(BuildContext context, TransactionModel t, NumberFormat fmt) {
    showModalBottomSheet(
      context: context,
      shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
          ...
          children: [
            ...
            Container(
              ...
              child: Icon(_txIcon(t.type),
                  color: t.type.isCredit ? AppColors.success : AppColors.error, size: 28),
            ),
            const SizedBox(height: 12),
            Text(
            ...
            Text(t.type.label, style: const TextStyle(color: AppColors.textHint, fontSize: 13)),
            const SizedBox(height: 20),
            _DetailRow(label: 'Nội dung', value: t.description ?? '—'),
            if (t.bookingCode != null) _DetailRow(label: 'Mã booking', value: t.bookingCode!),
            _DetailRow(label: 'Sau GD', value: fmt.format(t.balanceAfter)),
            _DetailRow(
                label: 'Thời gian', value: DateFormat('HH:mm dd/MM/yyyy').format(t.createdAt)),
  ...
  Widget _buildEmpty() => const Center(
        child: Column(
          ...
          children: [
            Icon(Icons.receipt_long_rounded, size: 60, color: AppColors.primaryLightBrand),
            const SizedBox(height: 12),
...
class _QuickAction extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
              color: AppColors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(10)),
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
...
class _DateSeparator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final label = isToday ? 'Hôm nay' : isYesterday ? 'Hôm qua' : DateFormat('dd/MM/yyyy').format(date);
...
class _PaymentOption extends StatelessWidget {
  ...
  final Color color;
  final IconData icon;
  const _PaymentOption({required this.label, required this.color, required this.icon});
