...
// ════════════════════════════════════════════════════════════════
import 'package:dat_san_247_mobile/core/base/errors/exceptions.dart';
...
mixin BaseNotifier<T> {
  ...
  Future<T?> runUnwrap<R extends Object>({
    ...
    action: () async {
      ...
      if (!res.isSuccess || res.data == null) {
        throw ServerException(message: res.message ?? 'Đã có lỗi xảy ra');
      }
      return mapper(res.data as R);
    },
  ...
  Future<T?> runResult<R>({
    ...
    action: () async {
      final result = await action();
      return result.fold(onSuccess: mapper, onFailure: (f) => throw f);
    },
    cancelPrevious: cancelPrevious,
    keepPreviousOnLoading: keepPreviousOnLoading,
    emitEmptyForEmptyList: emitEmptyForEmptyList,
