...
Future<void> loadCategories() async {
  state = const AsyncLoading();
  state = await runWithLoading(() => _repository.getCategories());
}
```
...
### 1. `runWithLoading()` - Cho Query Operations (GET/FETCH)
...
Future<void> loadCategories({Map<String, dynamic>? params}) async {
  state = const AsyncLoading();
  state = await runWithLoading(() => _repository.getCategories(params: params));
}
```
...
### 2. `runWithCallback()` - Cho Mutation Operations (POST/PUT/DELETE)
...
1. **Sử dụng `runWithLoading()` cho queries:**
```dart
Future<void> loadItems() async {
  state = const AsyncLoading();
  state = await runWithLoading(() => _repository.getItems());
}
```

2. **Sử dụng `runWithCallback()` cho mutations:**
```dart
...
```dart
// ❌ BAD
state = await runWithLoading(() => someNonResultFunction());
...
```dart
import 'package:flutter_base_template/core/state/riverpod/result_handler.dart';
...
class MyNotifier extends _$MyNotifier {
  ...
  Future<void> loadItems({Map<String, dynamic>? params}) async {
    state = const AsyncLoading();
    state = await runWithLoading(() => _repository.getItems(params: params));
  }
