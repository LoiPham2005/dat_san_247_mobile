...
/// Tập hợp các BlocListener helpers phổ biến
class BlocListeners {
  /// Listener chuẩn: hiển thị SnackBar khi failure (có error) hoặc success (có message)
  ///
  /// Màu sắc lấy từ Theme hiện tại — không hardcode.
  static BlocListener<B, BaseState<T>>
  common<B extends StateStreamable<BaseState<T>>, T>({
  ...
  static BlocListener<B, BaseState<T>>
  onError<B extends StateStreamable<BaseState<T>>, T>({
    required void Function(BuildContext context, String error, T? previousData)
    onFailure,
    Widget? child,
  }) {
    return BlocListener<B, BaseState<T>>(
  ...
  static BlocListener<B, BaseState<T>>
  onSuccess<B extends StateStreamable<BaseState<T>>, T>({
    required void Function(BuildContext context, T data, String? message)
    onSuccess,
    Widget? child,
  }) {
    return BlocListener<B, BaseState<T>>(
