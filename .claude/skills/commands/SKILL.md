...
| `🎨 Theme Generator` | `make theme-gen / expand / sync / preview` |
...
### `tools/theme_gen.dart`
Đọc `lib/design/theme/colors/color_config.json`, sinh ra 2 file:
- `lib/gen/theme/color_tokens.dart`
- `lib/gen/theme/color_palettes.dart`

```bash
# Gen code (keys phải đồng đều giữa các theme)
fvm dart run tools/theme_gen.dart
make theme-gen

# Thêm token mới từ meta vào tất cả themes rồi gen
fvm dart run tools/theme_gen.dart --expand
make theme-expand

# Sync keys từ theme 'light' sang các theme khác rồi gen
fvm dart run tools/theme_gen.dart --sync
make theme-sync

# Dry run — xem output, không ghi file
